import { PrismaClient, Role, FormCategory } from '@prisma/client';
...
function parseDepartment(entry: string) {
  const match = entry.match(/^(\d+)\s*-\s*(.+)$/);
  if (!match) return { code: entry, name: entry };
  return { code: match[1].trim(), name: match[2].trim() };
}

async function main() {
  // Seed departments
  const allDepts: { code: string; name: string; category: string }[] = [];
  ...
  // Seed form templates
  // FM-04: Room temperature / medication storage form
  await prisma.formTemplate.upsert({
    ...
    create: {
      ...
      description:
        'Form for checking temperature and humidity of medication storage areas. Normal room temp: 15-24°C, Relative humidity: 30-60%',
      category: FormCategory.ROOM_TEMPERATURE,
      fields: {
        sections: [
          {
            key: 'room',
  ...
  // FM-02.2: Fridge temperature chart
  await prisma.formTemplate.upsert({
    ...
    create: {
      ...
      description:
        'Daily refrigerator temperature monitoring chart. Normal range: 2-8°C. Check twice daily at 07:00 and 19:00.',
      category: FormCategory.FRIDGE_TEMPERATURE,
      fields: {
